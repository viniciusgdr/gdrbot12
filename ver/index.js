	...
	case 'kbbi':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 TR. GRP. 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
							sendMess(_.jid, `*「 TR. GRP. 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
						}
	    ...
	    case 'kapankah':
			...
			case 'closegc':
					...
					var nomor = mek.participant
					const close = {
					text: `Grupo fechado pelo administrador agora`,
					contextInfo: { mentionedJid: [nomor] }
					}
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
                reply(mess.wait)
                play = body.slice(5)
                anu = await fetchJson(`https://api.zeks.xyz/api/ytmp3?q=${play}&apikey=apivinz`)
               if (anu.error) return reply(anu.error)
                 infomp3 = `*ESPERE ENVIANDO, POR FAVOR*`
                buffer = await getBuffer
                lagu = await getBuffer
                client.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
